...
export abstract class BaseAddMemberModalViewModel<TRole extends string> {
  ...
  constructor(
    ...
    private readonly defaultRole: TRole,
  ) {
    this._selectedRole = defaultRole
    makeAutoObservable(this, {}, { autoBind: true })
  }

  protected abstract addUser(userId: string, roleId: TRole): Promise<boolean>
  ...
  public setSelectedRole(role: TRole): void {
  ...
  public async addSelectedUser(): Promise<void> {
    ...
    try {
      const success = await this.addUser(this._selectedUserId, this._selectedRole)
  ...
  public async createAndAddUser(): Promise<void> {
    ...
    try {
      ...
      const success = await this.addUser(newUser.node.id, this._selectedRole)
  ...
  private reset(): void {
    ...
    this._selectedRole = this.defaultRole
    this._newUsername = ''
    this._newPassword = ''
    this._newEmail = ''
    this._isAdding = false
