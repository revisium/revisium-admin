...
class TestStackItem extends StackItem<TestState> {
  ...
  constructor(state: TestState = { type: 'list' }) {
    super(state)
  }
...
class TestStackManager extends StackManager<TestStackItem, TestPayload, TestResult> {
  public lastRequestPayload: TestPayload | null = null

  protected createItemForRequest(request: StackRequest<TestPayload, TestResult>): TestStackItem {
    this.lastRequestPayload = request.payload
    return new TestStackItem({ type: 'selecting' })
  }
}
...
describe('StackManager', () => {
  describe('initialization', () => {
    it('should create with first item', () => {
      const firstItem = new TestStackItem({ type: 'root' })
  ...
  describe('pushRequest', () => {
    ...
    it('should create new item with selecting state', () => {
      const firstItem = new TestStackItem()
      const manager = new TestStackManager(firstItem)

      manager.pushRequest(firstItem, { query: 'test' }, jest.fn(), jest.fn())

      expect(manager.stack[1].state).toEqual({ type: 'selecting' })
    })
  })
  ...
  describe('resolveRequest', () => {
    it('should call resolve callback on parent', () => {
      ...
      manager.resolveRequest(selectingItem, 'selected-value')
    ...
    it('should handle nested requests', () => {
      ...
      manager.resolveRequest(level2Item, 'result-2')

      expect(resolve2).toHaveBeenCalledWith('result-2')
      expect(manager.stack).toHaveLength(2)

      manager.resolveRequest(level1Item, 'result-1')
  ...
  describe('edge cases', () => {
    it('should handle resolveRequest with item not in stack', () => {
      const firstItem = new TestStackItem()
      const manager = new TestStackManager(firstItem)
      const orphanItem = new TestStackItem()

      expect(() => manager.resolveRequest(orphanItem, 'result')).not.toThrow()
    })
    ...
    it('should handle deep nesting', () => {
      ...
      manager.resolveRequest(middleItem, 'middle-result')
